...
import { CheckCircle2, XCircle, Terminal, FileText, Globe, Search } from 'lucide-react'
...
export default function ToolExecutionCard({ tool }: { tool: ToolResult }) {
  ...
  const toolLabel = isTerminal ? tool.command : isFile ? tool.filename : isSearch ? tool.query : tool.tool_name
  const ToolIcon = isTerminal ? Terminal : isFile ? FileText : isSearch ? Search : Globe
